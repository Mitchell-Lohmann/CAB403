...
int main(int argc, char **argv)
{
    ...
    int tempsensor_port = splitAddressPort(full_addr, tempsensor_addr);
    ...
    for (int i = 0; i < receiverNum; i++)
    {
        ...
        receiver_port[i] = splitAddressPort(receiver_full_addr, receiver_addr);
    }
    ...
    shm_tempsensor *shared = (shm_tempsensor *)(shm + shm_offset);
    ...
    addr.sin_port = htons(tempsensor_port);
    ...
    { // Loop starts
        ...
        // If the temperature has changed OR if this is the first iteration of this loop OR if the max update wait has passed since this sensor last sent an update
        if (loopCount == 1 || tempReading != shared->temperature || elapsed_time >= max_update_wait)
        {
            ...
            for (int i = 0; i < receiverNum; i++)
            {
                ...
                destaddr.sin_port = htons(receiver_port[i]);
                if (inet_pton(AF_INET, receiver_addr, &destaddr.sin_addr) != 1)
                {
        ...
        /* Debug */
        // printf("Pthread_mutex_lock by %d\n", id);
        ...
        /* Debug */
        // printf("Waited for condvar to be signaled by %d\n", id);
